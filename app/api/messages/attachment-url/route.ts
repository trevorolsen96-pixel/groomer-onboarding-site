...
// Short-lived on purpose — this is only used to render the image inline in
// the app's own message history right now, not to hand out a long-lived
// link. The customer-facing link sent by SMS has its own separate 72-hour
// expiry set at send time in /api/messages/send.
...
export async function GET(request: Request) {
  try {
    ...
    // Every attachment path is namespaced as `{businessId}/...` at upload
    // time — this stops one business from requesting a signed link for
