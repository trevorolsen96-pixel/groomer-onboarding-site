...
export async function POST(request: Request) {
  try {
    ...
    const rows: GroomerIORow[] = body.rows;
    ...
    for (const row of rows) {
      ...
      // Duplicate check by phone number -- same rule the MoeGo importer
      // uses, so re-running either importer (or both, for a business
      // that's tried more than one export) against the same business
      // converges instead of creating repeat clients.
      ...
      const { error: customerError } = await supabaseAdmin
        ...
        .insert({
          ...
          secondary_contact_phone: normalizePhone(row.secondaryPhone),
          // Groomer.io's Client Summary export has no active/inactive
          // status column at all (unlike MoeGo's), so every import lands
          // as active -- there's no source data to decide otherwise.
          is_active: true,
          deleted: false,
