...
// Short-lived on purpose -- only for viewing a document inside the app.
...
export async function GET(request: Request) {
  try {
    ...
    // Every pet document path is namespaced as `{businessId}/...` at
    // upload time -- stops one business requesting a signed link for
    // another business's document just by guessing/passing a different
    // path.
    ...
    if (signError || !signedData?.signedUrl) {
      return NextResponse.json(
        { error: signError?.message ?? "Unable to load document." },
        { status: 400 }
      );
    }
